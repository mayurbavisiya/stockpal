...
public class JwtTokenUtil implements Serializable {
	...
	private JwtResponse doGenerateToken(Map<String, Object> claims, String subject) {
		...
		claims.put("fullame", user.getFullName());
		claims.put("username", user.getUsername());
		claims.put("type", user.getUsertype());
		claims.put("id", user.getId());
		String jwtToken = Jwts.builder().setClaims(claims).setSubject(subject)
				.setIssuedAt(new Date(System.currentTimeMillis()))
				.setExpiration(new Date(System.currentTimeMillis() + JWT_TOKEN_VALIDITY * 56 * 1000))
				.signWith(SignatureAlgorithm.HS512, secret).compact();
		JwtResponse jwtRes = new JwtResponse(jwtToken, subject, user.getFullName(), user.getUsertype(), user.getId());
		return jwtRes;
	}
